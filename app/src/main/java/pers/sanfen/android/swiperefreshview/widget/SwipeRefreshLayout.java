...
public class SwipeRefreshLayout extends ViewGroup implements NestedScrollingParent,
    ...
    private void addDefaultHeadView(Context context) {
        mHeadView = LayoutInflater.from(context).inflate(R.layout.layout_pull_to_refresh_header, null);
        mRefreshTitle = mHeadView.findViewById(R.id.tv_refresh);
        mLoadView = mHeadView.findViewById(R.id.load_view);
        addView(mHeadView, 0);
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        if (mCurrentTargetOffsetTop < 0) mCurrentTargetOffsetTop = 0;
    ...
    private void updateContentOffsetTop(int targetTop) {
        ...
        if (currentTop > mDistanceToTriggerSync) {
            ...
            if (!mCanRefreshing && !mRefreshing) {
                ...
                if (mIsDefaultHeadView) {
                    mRefreshTitle.setText(getResources().getString(R.string.loosen_refresh));
                    mLoadView.cancelAnim();
                }
            }
            mCanRefreshing = true;
        } else {
            ...
            if (!mIsPulling && !mRefreshing) {
                ...
                if (mIsDefaultHeadView) {
                    mRefreshTitle.setText(getResources().getString(R.string.pull_refresh));
                    mLoadView.cancelAnim();
                }
            }
            mIsPulling = true;
    ...
    private void finishSpinner(float overscrollTop) {
        if (overscrollTop / 2 > mDistanceToTriggerSync) {
            startRefresh();
        } else {
            removeCallbacks(mCancel);
            post(mCancel);
...
//        if (removeCallBack && mTarget.getTop() == getPaddingTop()) {
